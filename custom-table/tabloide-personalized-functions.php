...
require_once("personnalized.php");
    ...
    function image_size(){
        add_image_size("custom_table_default_image", 1920, 1080, false);
    }
        ...
        if (!empty($file) && !empty($picture)) {
            ...
            if ($width > $height) {
                while ($width > 1920) {
                    ...
                    $imgeResize = imagescale(imagecreatefromjpeg($picture), $resize);
                    imagejpeg($imgeResize, $uploadfile);
            ...
            if ($width >= 500 && $width <= 1920 && $height >= 500 && $height <= 1080 ) {
